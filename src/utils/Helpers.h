 ...
 * @date 08/30/2023
 * @brief This file contains helper functions and structures used throughout the
 * program.
 */
...
/**
 * @brief This function replaces the value of a specified option in a map with a
 * new value.
 *
 ...
 *
 * @note This function uses the boost library's any class to allow for type
 * erasure, meaning the function can accept any type for the new value.
 */
template <class T>
void replace(std::map<std::string, boost::program_options::variable_value> &vm,
             const std::string &opt, const T &val) {
    vm[opt].value() = boost::any(val);
}

/**
 * @brief Synchronizes the values of a given key between a JSON object and a
 * Boost variables_map object.
 *
 * This function first checks if the key exists in the variables_map. If it
 * does, the value is updated in the JSON object. Then, if the key exists in the
 * JSON object, the value is read from the JSON object and updated in the
 * variables_map.
 *
 * @tparam T The type of the value associated with the key.
 * @param key The key to synchronize between the JSON object and the
 * variables_map.
 ...
 */
template <typename T>
void syncMapKeys(std::string &key, nlohmann::json &inputMap,
    ...
    // then, if key is in the JSON map, read value from the JSON map into the
    // variables_map.
...
/**
 * @brief This function fills a vector with evenly spaced values between a start
 * and end value.
 *
...
/**
 * @brief Calculates the number of iterations for a given point in the Julia
 * set.
 ...
 */
template <typename T>
size_t juliaSetIterations(const std::complex<T> &z0, const std::complex<T> &c,
 ...
 *
 * @tparam T This is the type of the scaling factor and the growth rate. It can
 * be any type that supports assignment and arithmetic operations.
 */
template <typename T> struct ToneMap {
     ...
     * It is initialized to a very small value (1e-7) to prevent large initial
     * tones.
     */
     ...
     * It is initialized to a moderate value (0.3) to allow for a reasonable
     * growth rate.
     */
    T growth_rate = 0.3;
};
...
/**
 * @brief A struct representing the properties of a canvas for rendering ANSI
 * characters
 * @tparam T The type of the numbers used for the canvas dimensions and
 * properties.
 */
 ...
 */
static std::vector<std::string> getGrays256bitANSI(double scaling_factor = 1e-7,
...
static std::vector<std::string> getHSV256bitANSI() {
    ...
    std::vector<std::string> hues = {
        "225", "219", "218", "211", "212", "213", "207", "206", "200", "201",
        "165", "129", "093", "057", "021", "027", "033", "039", "045", "051",
        "087", "050", "049", "086", "085", "048", "047", "084", "083", "046",
        "082", "119", "155", "118", "154", "191", "190", "226", "227", "221",
        "220", "214", "208", "202", "160", "196", "197", "198", "197",
    };
...
/**
 * @brief Prints the Julia set to the console using the specified canvas
 * properties.
 ...
 */
template <typename T>
void printJuliaSet(const Canvas &canvas, const T x0, const T y0,
                   const size_t max_iterations = 120) {
    ...
    for (size_t y = 0; y < canvas.height; ++y) {
        for (size_t x = 0; x < canvas.width; ++x) {
            std::complex<T> z = std::complex<T>(canvas.x_start + x * x_step,
                                                canvas.y_start + y * y_step);
            size_t iter = juliaSetIterations(z, c, max_iterations);
            const T log_iter = std::log2(iter);
            if (log_iter == NAN ||
                std::isinf(static_cast<long double>(log_iter))) {
                continue;
            }
    ...
    std::vector<std::string> colors = getGrays256bitANSI(
        canvas.tone_map.scaling_factor, canvas.tone_map.growth_rate);
    for (size_t y = 0; y < canvas.height; ++y) {
        for (size_t x = 0; x < canvas.width; ++x) {
            std::complex<T> z = std::complex<T>(canvas.x_start + x * x_step,
                                                canvas.y_start + y * y_step);
            ...
            T color_index =
                scale<T>(enhanced_value, min, max, 0, colors.size());
            size_t clamped_index = std::clamp<size_t>(
                static_cast<size_t>(color_index), 0, colors.size() - 1);
