 ...
 * @date 09/30/2023
 * @brief This file contains the implementation of the Successive
 * Over-Relaxation (SOR) method for solving systems of linear equations.
 */
...
/**
 * @brief This function applies the Successive Over-Relaxation (SOR) method to
 ...
 * @param tolerance The tolerance for convergence. The method stops when the
 * error is less than this value.
 * @param relaxation_factor The relaxation factor to use in the SOR method. This
 * value is typically between 0 and 2.
...
applySOR(const MyBLAS::Matrix<T> &A, const MyBLAS::Vector<T> &b,
    ...
    for (results.iterations = 0; results.iterations < max_iterations;
        ...
        for (size_t row = 0; row < n; row++) {
            ...
            // Update the solution vector with the new value, including the
            // relaxation factor
