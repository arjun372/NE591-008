...
protected:
   ...
   */
    ...
    void performInputArgumentsCheck(boost::program_options::variables_map &map) override {
        ...
        // FILE OPTIONAL
        // if a filepath for an input parameter json is not provided, parameters will be read from the terminal interactively.
        if(!map.count("input-parameter-json") && !quietMode) {
            std::cout<<"INFO: Input parameter JSON filepath not provided. Parameters will be specified interactively\n";
        }
        // else:
        ...
        // If the path is invalid, a new path will be accepted interactively
            ...
            while(map["input-parameter-json"].empty() || !doesFileExist(map["input-parameter-json"].as<std::string>())) {
        ...
        // If the path is invalid, a new path will be accepted interactively
        if(!map.count("source-terms-csv") || map["source-terms-csv"].empty() || !doesFileExist(map["source-terms-csv"].as<std::string>())) {
            std::cerr << "INFO: Source-terms matrix cannot be read interactively.\n" << std::endl;
            while(!map.count("source-terms-csv") || map["source-terms-csv"].empty() || !doesFileExist(map["source-terms-csv"].as<std::string>())) {
        ...
        // If the path is invalid, a new path will be accepted interactively
            ...
            while(!map.count("output-results-json") || map["output-results-json"].empty() || !isFileWritable(map["output-results-json"].as<std::string>())) {
        ...
        // If the path is invalid, a new path will be accepted interactively
            ...
            while(!map.count("output-flux-csv") || map["output-flux-csv"].empty() || !isFileWritable(map["ooutput-flux-csv"].as<std::string>())) {
