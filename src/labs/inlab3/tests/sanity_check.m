% Define the parametric function f(x) = x^4 - x^3 - x^2 + x + 1
f = @(x) x.^4 - x.^3 - x.^2 + x + 1;

% Define the range of x values you want to plot over
xmin = -1.5; % Minimum x value
...
% Evaluate the function f(x) at the smooth curve points
y_smooth = f(x_smooth);

% Number of points to sample
n = 10;

% Generate n random x values in the specified range
x_samples = xmin + (xmax - xmin) * rand(1, n);

% Evaluate the function f(x) at the sampled x values
