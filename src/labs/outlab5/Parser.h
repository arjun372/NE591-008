...
class Parser : public CommandLine<InputMatrices> {
  ...
  protected:
    ...
    void buildInputArguments(
        ...
        values.add_options()("no-pivoting", "Do not perform partial pivoting")(
    ...
    printInputArguments(boost::program_options::variables_map &vm) override {
        ...
        std::cout << "\tUse Pivoting          : " << (noPivoting ? "No" : "Yes")
                  << "\n";
        std::cout << "\tUse Alternate Method  : "
                  << (alternateMethod ? "Yes" : "No") << "\n";
        CommandLine::printLine();
    }
     ...
     */
    void buildInputs(InputMatrices &inputs,
