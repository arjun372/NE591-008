  ...
  protected:
    ...
    static void printResults(InLab6Outputs &results) {
        ...
        std::cout << "\tabsolute maximum residual : "
                  << (results.getMaxResidual()) << std::endl;
     ...
     */
    void run(InLab6Outputs &outputs, InLab6Inputs &inputs,
        ...
        if (inputs.methods.count(
            ...
            pointJacobiResults.toJSON(
                results["outputs"][MyRelaxationMethod::TypeKey(
                    MyRelaxationMethod::Type::METHOD_POINT_JACOBI)]);
            Parser::printLine();
        ...
        if (inputs.methods.count(MyRelaxationMethod::Type::METHOD_SOR)) {
            Parser::printLine();
            std::cout << "Successive over-relaxation (SOR) not implemented yet."
                      << std::endl;
        }

        if (inputs.methods.count(MyRelaxationMethod::Type::METHOD_SSOR)) {
            Parser::printLine();
            std::cout << "Symmetric Successive over-relaxation (SOR) not "
                         "implemented yet."
                      << std::endl;
        }
