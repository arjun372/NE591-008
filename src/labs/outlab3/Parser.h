...
class Parser : public CommandLine<NewtonCotesInputs> {
  ...
  protected:
    ...
    void performInputArgumentsCheck(
        ...
        while (!gaussian_set) {
            try {
                std::cout << "Would you like to use Gauss-Legendre Quadrature? "
                             "[YES/no]: ";
                std::cin >> input;
        ...
        if (map["use-gaussian-quadratures"].as<bool>()) {
            while (map["num-quadrature-points"].empty() ||
                   failsNaturalNumberCheck(
                       map["num-quadrature-points"].as<long double>())) {
                std::cout
                    << "Enter the number of Gauss-Legendre Quadrature points: ";
                std::cin >> input;
                try {
    ...
    void buildInputs(NewtonCotesInputs &inputs,
        ...
        inputs.n = !values["num-quadrature-points"].empty()
                       ? static_cast<size_t>(
                             values["num-quadrature-points"].as<long double>())
                       : 0;
