...
typedef struct Input {
    ...
    size_t m = -1; ///< The number of sub-intervals to divide the integration
                   ///< interval into.
    size_t n = -1; ///< The number of quadrature points if Gauss-Legendre
                   ///< Quadrature will be used.
    bool flip_integral = false; ///< If true, the integral will be evaluated in
                                ///< reverse order (from b to a).
    std::set<IntegrationRule> integral_types = {
        RULE_NONE}; ///< A set of integration rules to be applied.
    /**
...
typedef struct Outputs {
     ...
     *
     * @param jsonMap A reference to a JSON object to be populated with the
     * data.
     */
    void toJSON(nlohmann::json &jsonMap) {
        jsonMap["integral"] = integral;
        jsonMap["weights"] = [this]() -> std::map<std::string, long double> {
            ...
            std::transform(weights.begin(), weights.end(),
                           ...
                           [i = 0](const long double &value) mutable {
                               return std::make_pair("w" + std::to_string(i++),
                                                     value);
                           });
            return result;
        }();
        jsonMap["quadrature-points"] =
            [this]() -> std::map<std::string, long double> {
            std::map<std::string, long double> result;
            std::transform(quadrature_points.begin(), quadrature_points.end(),
                           std::inserter(result, result.end()),
                           [i = 0](const long double &value) mutable {
                               return std::make_pair("n" + std::to_string(i++),
                                                     value);
                           });
            return result;
        }();
