...
class Parser : public CommandLine<OutLab6Inputs> {
  ...
  protected:
    ...
    void buildInputArguments(
        ...
        values.add_options()("threshold,t",
                             boost::program_options::value<long double>(),
                             "= convergence threshold [𝜀 > 0]")(
            "max-iterations,k", boost::program_options::value<long double>(),
            "= maximum iterations [n ∈ ℕ]")(
            "relaxation-factor,w", boost::program_options::value<long double>(),
            "= SOR weight, typical w ∈ [0,2]")(
            "order,n", boost::program_options::value<long double>(),
            "= order of the square matrix [n ∈ ℕ]")(
            "input-json,i", boost::program_options::value<std::string>(),
            "= input JSON containing A, and b")(
        ...
        methods.add_options()("use-LUP", "= Use LUP factorization")(
            "use-point-jacobi", "= Use the Point-Jacobi method")(
            "use-gauss-seidel", "= Use the Gauss-Seidel method")(
            "use-SOR", "= Use the SOR method")("use-SORJ",
                                               "= Use the SOR Jacobi method")(
    ...
    printInputArguments(boost::program_options::variables_map &vm) override {
        ...
        const auto inputJson = vm.count("input-json")
                                   ? vm["input-json"].as<std::string>()
                                   : "None provided";
        std::cout << "\tGenerate A,b,            g: " << (gen ? "Yes" : "No")
                  << "\n";
        std::cout << "\tInput JSON (for A, b),   i: "
                  << (gen ? "[IGNORED] " : " ") << inputJson << "\n";
        std::cout << "\tOutput JSON (for x),     o: "
                  << vm["output-json"].as<std::string>() << "\n";
        std::cout << "\tConvergence Threshold,   𝜀: "
                  << vm["threshold"].as<long double>() << "\n";
        std::cout << "\tMax iterations,          k: "
                  << static_cast<size_t>(vm["max-iterations"].as<long double>())
                  << "\n";
        std::cout << "\tMatrix order,            n: "
                  << (vm.count("order")
                          ? std::to_string(static_cast<size_t>(
                                vm["order"].as<long double>()))
                          : "None provided, will be inferred from input JSON")
                  << "\n";
        std::cout << "\tSOR weight,              ω: "
                  << (vm.count("relaxation-factor")
                          ? std::to_string(
                                vm["relaxation-factor"].as<long double>())
                          : "N/A")
                  << "\n";
        std::cout << "\tUse LUP factorization     : "
                  << (vm["use-LUP"].as<bool>() ? "Yes" : "No") << "\n";
        std::cout << "\tUse Gauss-Seidel          : "
                  << (vm["use-gauss-seidel"].as<bool>() ? "Yes" : "No") << "\n";
        std::cout << "\tUse Point-Jacobi          : "
                  << (vm["use-point-jacobi"].as<bool>() ? "Yes" : "No") << "\n";
        std::cout << "\tUse SOR                   : "
                  << (vm["use-SOR"].as<bool>() ? "Yes" : "No") << "\n";
        std::cout << "\tUse Point-Jacobi with SOR : "
                  << (vm["use-SORJ"].as<bool>() ? "Yes" : "No") << "\n";
        std::cout << "\tUse symmetric SOR         : "
                  << (vm["use-SSOR"].as<bool>() ? "Yes" : "No") << "\n";
        CommandLine::printLine();
    }
    ...
    void performInputArgumentsCheck(
        ...
        performChecksAndUpdateInput<long double>("threshold", inputMap, map,
                                                 checks);

        checks.emplace_back(
            [](long double value) { return failsWholeNumberCheck(value); });
        performChecksAndUpdateInput<long double>("max-iterations", inputMap,
                                                 map, checks);

        if (map.count("order")) {
            performChecksAndUpdateInput<long double>("order", inputMap, map,
                                                     checks);
        } else if (map.count("generate")) {
            checks.clear();
            checks.emplace_back([](long double value) {
                return failsNaturalNumberCheck(value);
            });
            performChecksAndUpdateInput<long double>("order", inputMap, map,
                                                     checks);
        }
        ...
        if (map["use-SOR"].as<bool>() || map["use-SORJ"].as<bool>() ||
            map["use-SSOR"].as<bool>()) {
            checks.clear();
            performChecksAndUpdateInput<long double>("relaxation-factor",
                                                     inputMap, map, checks);
        }
    }
     ...
     */
    void buildInputs(OutLab6Inputs &inputs,
        ...
        if (generate) {
            if (!values.count("order")) {
                std::cerr
                    << "Error: User did not provide matrix order n, aborting\n";
                exit(-1);
            }
            auto n = static_cast<size_t>(values["order"].as<long double>());
            inputs.input.n = n;
            MyBLAS::System::Circuit(n, inputs.input.coefficients,
                                    inputs.input.constants,
                                    inputs.known_solution);
        } else {
            // read the constants
            inputs.input.constants = MyBLAS::Vector(MyBLAS::Vector(
                std::vector<long double>(inputMap["constants"])));
            // read the coefficient matrix
            inputs.input.coefficients =
                MyBLAS::Matrix(std::vector<std::vector<long double>>(
                    inputMap["coefficients"]));
        }

        if (!MyBLAS::isSquareMatrix(inputs.input.coefficients)) {
            std::cerr
                << "Error: Input coefficients matrix A not square, aborting.\n";
            exit(-1);
        }

        if (inputs.input.coefficients.getRows() !=
            inputs.input.constants.size()) {
            std::cerr
        ...
        if (!generate) {
            // set input order n
            const auto orderFromInputMatrixDimensions =
                inputs.input.coefficients.getRows();
            if (!values.count("order")) {
                std::cout
                ...
                inputs.input.n = orderFromUser > orderFromInputMatrixDimensions
                                     ? orderFromInputMatrixDimensions
                                     : orderFromUser;
                if (orderFromUser > orderFromInputMatrixDimensions) {
                    std::cerr << "Warning: Matrix order (n) is larger than "
                                 "input matrix, defaulting to lower value\n";
                }
            }
        }

        inputs.input.threshold = values["threshold"].as<long double>();
        inputs.input.max_iterations =
            static_cast<size_t>(values["max-iterations"].as<long double>());
        ...
        if (values.count("relaxation-factor")) {
            inputs.input.relaxation_factor =
                values["relaxation-factor"].as<long double>();
        }
