function [F] = plot_points(n, m, x, xi, L_xi, f_xi, E_xi)
F = figure;
hold on;
...
grid minor
box on
% Plot f(x) as a smooth line
plot(xi, f_xi, 'LineWidth', 2);
% Plot L(x) as points with E(x) as the error bars
errorbar(xi, L_xi, E_xi, ".", 'LineWidth', 0.75, 'Color','k');
...
for i = 1:n
    y = diff(y) ./ diff(x);
    x = x(1:end-1); % adjust the x values
end


% % Define the function and its derivatives
% f = @(x) exp(x);
% df = @(x) exp(x); % derivative of e^x is e^x
% ddf = @(x) exp(x); % second derivative of e^x is e^x
% dddf = @(x) exp(x); % third derivative of e^x is e^x
% ddddf = @(x) exp(x); % fourth derivative of e^x is e^x
% 
% for(i = 0; i <= n; i++)
% Define the nodes
x_nodes = linspace(-1, 1, n); % change this to your actual nodes
% Compute the maximum of the fourth derivative in the interval [-1, 1]
z = xi'; % linspace(-1, 1, m);
max_ddddf = max(y);
